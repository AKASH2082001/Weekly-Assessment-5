...
crime.config["SESSION_PERMANENT"] = False
crime.config["SESSION_TYPE"] = "filesystem"
Session(crime)
...
@crime.route('/sort',methods=['GET','POST'])
def Search_crime():
    if request.method == 'POST':
...
@crime.route('/user',methods=['GET','POST'])
def Login_user():
    if request.method == 'POST':
        getEmail = request.form["useremail"]
        getPass = request.form["userpass"]
...
@crime.route('/userdashboard')
def user_dashboard():
    return render_template("userdashboard.html")

@crime.route('/usersession')
def userpage():
...
@crime.route('/complaint',methods=['GET','POST'])
def Report_crime():
    if request.method == 'POST':
...
@crime.route('/update',methods=['GET','POST'])
def Update_user():
    try:
        if request.method == 'POST':
...
@crime.route('/edit',methods=['GET','POST'])
def User_edit():
    if request.method == 'POST':
        getName = request.form["newname"]
